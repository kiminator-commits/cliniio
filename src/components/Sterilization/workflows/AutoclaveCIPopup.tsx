...
interface AutoclaveCIPopupProps {
  isOpen: boolean;
  onClose: () => void;
...
export const AutoclaveCIPopup: React.FC<AutoclaveCIPopupProps> = ({
  isOpen,
  onClose,
  currentCycleId,
  facilityId,
  ...
  const handleResponse = async (ciPassed: boolean) => {
    try {
      ...
      await batchCiConfirmation({
        ...
        ciPassed,
        notes: ciPassed
          ? 'CI strips passed and packages free of moisture.'
          : 'CI strips failed or packages contain moisture.',
      });

      if (!ciPassed) {
        toast.error('⚠️ CI failed — check sterilizer and packaging integrity.');
      } else {
        toast.success('✅ CI verification recorded.');
      }

      onClose();

      // Call completion callback to proceed with cycle completion
      if (onComplete) {
        onComplete();
      }
    } catch (error) {
      console.error('Error saving CI verification:', error);
      toast.error('Error saving CI verification.');
    } finally {
        ...
        >
          ...
          >
            ...
            <div className="bg-[#4ECDC4] text-white px-6 py-4">
              <h2 className="text-xl font-semibold">CI Verification</h2>
            </div>

            {/* Content */}
            <div className="flex flex-col items-center justify-center text-center px-6 py-8">
              <p className="text-lg font-medium mb-6 text-gray-800">
                Did all CI strips pass, and were packages free of moisture?
              </p>
