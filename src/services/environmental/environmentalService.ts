...
export const environmentalService = {
  async processScan(code: string) {
    try {
      ...
      if (userError || !user) {
        console.error(
          'Unauthorized: cannot process environmental scan without a valid session.'
        );
      ...
      const { error } = await supabase
        .from('environmental_clean_audit_logs')
        .insert([auditEntry]);
      ...
      console.error(
        '❌ Failed to persist environmental scan audit log:',
