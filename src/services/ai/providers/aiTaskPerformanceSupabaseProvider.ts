 ...
 */
export async function getTaskById(taskId: string): Promise<DailyOperationsTaskRow | null> {
  const { data: task, error: taskError }: {
    data: Database['public']['Tables']['home_daily_operations_tasks']['Row'] | null;
    error: PostgrestError | null;
  } = await supabase
    .from<Database['public']['Tables']['home_daily_operations_tasks']['Row']>('home_daily_operations_tasks')
    .select('*')
    .eq('id', taskId)
    .single();
...
export async function insertTaskPerformance(
  ...
  const timeSaved = calculateTimeSaved(performance.estimatedDuration, performance.actualDuration);

  const { error }: { error: PostgrestError | null } = await supabase
    .from<Database['public']['Tables']['ai_task_performance']['Row']>('ai_task_performance')
    .insert([
      {
...
export async function updateDailyTimeSaved(
  ...
  const { error: updateError }: { error: PostgrestError | null } = await supabase
    .from<Database['public']['Tables']['performance_metrics']['Row']>('performance_metrics')
    .update({
...
export async function getCurrentUserGamificationStats(
  ...
  const { data: currentStats }: {
    ...
    .from<Database['public']['Tables']['user_gamification_stats']['Row']>('user_gamification_stats')
    .select('*')
    .eq('user_id', userId)
    .eq('facility_id', facilityId)
    .eq('date', today)
...
export async function getRawTaskDeltasForFacility(facilityId: string): Promise<{
  ...
  return data.map((row) => {
    const rowData = row as Database['public']['Tables']['ai_task_performance']['Row'];
    return {
      id: rowData.id ?? '',
 ...
 */
export async function getLearningProgressData(userIds: string[]): Promise<Database['public']['Tables']['user_learning_progress']['Row'][]> {
  const { data: learningData, error: _learningError }: {
    data: Database['public']['Tables']['user_learning_progress']['Row'][] | null;
    error: PostgrestError | null;
  } = await supabase
    .from<Database['public']['Tables']['user_learning_progress']['Row']>('user_learning_progress')
 ...
 */
export async function getInventoryCheckData(facilityId: string): Promise<Database['public']['Tables']['inventory_checks']['Row'][]> {
  const { data: inventoryData, error: _inventoryError }: {
    data: Database['public']['Tables']['inventory_checks']['Row'][] | null;
    error: PostgrestError | null;
  } = await supabase
    .from<Database['public']['Tables']['inventory_checks']['Row']>('inventory_checks')
    .select('accuracy')
    .eq('facility_id', facilityId);
 ...
 */
export async function getSterilizationCycleData(facilityId: string): Promise<Database['public']['Tables']['sterilization_cycles']['Row'][]> {
  const { data: sterilizationData, error: _sterilizationError }: {
    data: Database['public']['Tables']['sterilization_cycles']['Row'][] | null;
    error: PostgrestError | null;
  } = await supabase
    .from<Database['public']['Tables']['sterilization_cycles']['Row']>('sterilization_cycles')
    .select('status')
    .eq('facility_id', facilityId);
