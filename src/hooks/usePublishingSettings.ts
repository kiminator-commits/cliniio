...
interface PublishingSettingsState {
  ...
  updateSetting: <K extends keyof PublishingSettings>(
    key: K, 
...
const getStorageKey = (facilityId: string) => `publishingSettings_${facilityId}`;

const usePublishingSettingsStore = create<PublishingSettingsState>((set, get) => ({
...
// Hook that provides facility-aware publishing settings
export const usePublishingSettings = () => {
  const { getCurrentFacilityId, isLoading: facilityLoading } = useFacility();
  const facilityId = getCurrentFacilityId();
  
  const store = usePublishingSettingsStore();
  ...
  React.useEffect(() => {
    ...
    if (facilityId) {
      ...
      if (!isDevMode) {
        console.warn('No facility ID available, using default publishing settings');
      }
  ...
  const updateSetting = React.useCallback(<K extends keyof PublishingSettings>(
    key: K, 
    value: PublishingSettings[K]
  ) => {
