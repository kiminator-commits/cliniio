...
export function useWorkflowScanner() {
  const handleScan = useCallback(async (scannedCode: string): Promise<ScanResult> => {
    ...
    try {
      ...
      console.error("Error processing scan:", err instanceof Error ? err.message : String(err));
      return { type: "unknown", code: scannedCode, timestamp };
    }
  }, []);
