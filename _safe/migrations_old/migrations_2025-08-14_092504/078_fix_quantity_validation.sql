-- Migration: Fix quantity validation logic
-- This fixes the issue where current quantity validation happens before default values are set

-- Drop the existing trigger first
DROP TRIGGER IF EXISTS validate_inventory_item_trigger ON inventory_items;

-- Update the validation function to fix quantity validation order
...
BEGIN
  ...
  IF NEW.maximum_quantity < 999999 AND NEW.current_quantity > NEW.maximum_quantity THEN
